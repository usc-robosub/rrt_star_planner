...
RRTNode::RRTNode(const geometry_msgs::Point &pos, std::shared_ptr<RRTNode> p,
                 double c)
    : position(pos), parent(p), cost(c) {}

// RRTStarPlanner implementation
RRTStarPlanner::RRTStarPlanner()
    : nh_(), pnh_("~"), pose_received_(false), gen_(std::random_device{}()) {
  ...
  x_dist_ = std::uniform_real_distribution<double>(sampling_bounds_[0][0],
                                                   sampling_bounds_[0][1]);
  y_dist_ = std::uniform_real_distribution<double>(sampling_bounds_[1][0],
                                                   sampling_bounds_[1][1]);
  z_dist_ = std::uniform_real_distribution<double>(sampling_bounds_[2][0],
                                                   sampling_bounds_[2][1]);
  goal_bias_dist_ = std::uniform_real_distribution<double>(0.0, 1.0);

  // Subscribers
  pose_sub_ = nh_.subscribe("odometry/filtered/global", 1,
                            &RRTStarPlanner::poseCallback, this);

  // Service
  plan_service_ =
      nh_.advertiseService("plan_path", &RRTStarPlanner::planPathService, this);
  // Collision check service client
  pnh_.param<std::string>("check_collision_service", collision_service_name_,
                          std::string("check_collision"));
  collision_client_ = nh_.serviceClient<barracuda_msgs::CheckCollision>(
      collision_service_name_);
  ...
  marker_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("planning_markers", 1);

  ROS_INFO("RRT* 3D Planner initialized");
}

void RRTStarPlanner::poseCallback(
    const nav_msgs::Odometry::ConstPtr &msg) {
  current_odom_ = *msg;
  pose_received_ = true;
}

// No pointcloud callback; rely on collision service only
...
bool RRTStarPlanner::planPathService(barracuda_msgs::PlanPath::Request &req,
  ...
  // Collision checking uses service; no pointcloud required

  // Extract start and goal positions
  geometry_msgs::Point start = current_odom_.pose.pose.position;
  geometry_msgs::Point goal = req.goal_pose.pose.position;

  ROS_INFO("Planning from (%.2f, %.2f, %.2f) to (%.2f, %.2f, %.2f)", start.x,
           start.y, start.z, goal.x, goal.y, goal.z);
  ...
  waypoints_msg.header.frame_id = current_odom_.header.frame_id;
...
RRTStarPlanner::planPath(const geometry_msgs::Point &start,
  ...
  for (int iter = 0; iter < max_iterations_; ++iter) {
    ...
    geometry_msgs::Point new_point =
        steer(nearest_node->position, random_point);
    ...
    // Create new node
    double new_cost =
        nearest_node->cost + distance(nearest_node->position, new_point);
    auto new_node =
        std::make_shared<RRTNode>(new_point, nearest_node, new_cost);

    // Find nearby nodes for rewiring
    std::vector<std::shared_ptr<RRTNode>> nearby_nodes =
        findNearbyNodes(nodes, new_point, rewire_radius_);
    ...
    for (auto &near_node : nearby_nodes) {
      ...
      double cost =
          new_node->cost + distance(new_node->position, near_node->position);
      if (cost < near_node->cost &&
          isCollisionFree(new_node->position, near_node->position)) {
        // Remove from old parent's children
        if (near_node->parent) {
          auto &children = near_node->parent->children;
          children.erase(
              std::remove(children.begin(), children.end(), near_node),
              children.end());
        }
...
geometry_msgs::Point
RRTStarPlanner::samplePoint(const geometry_msgs::Point &goal) {
...
std::shared_ptr<RRTNode> RRTStarPlanner::findNearestNode(
...
std::vector<std::shared_ptr<RRTNode>> RRTStarPlanner::findNearbyNodes(
    const std::vector<std::shared_ptr<RRTNode>> &nodes,
    const geometry_msgs::Point &point, double radius) {
...
bool RRTStarPlanner::isCollisionFree(const geometry_msgs::Point &from,
  ...
  for (int i = 0; i <= num_checks; ++i) {
    ...
    if (!collision_client_.call(srv)) {
      ROS_WARN_THROTTLE(
          1.0, "Collision service '%s' call failed; assuming collision.",
          collision_service_name_.c_str());
      return false;
    }
...
std::vector<geometry_msgs::Point>
RRTStarPlanner::smoothPath(const std::vector<geometry_msgs::Point> &path) {
  if (path.size() <= 2) {
    return path;
  }
...
void RRTStarPlanner::visualizeTree(
    const std::vector<std::shared_ptr<RRTNode>> &nodes,
  ...
  edges.header.frame_id = current_odom_.header.frame_id;
